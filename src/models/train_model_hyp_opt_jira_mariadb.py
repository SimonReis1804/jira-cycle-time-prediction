...
from src.data.daten_laden_mariadb import lade_daten_mariadb
...
def train_model(model_name: str):
    """
    Trainiert ein Modell auf Jira-Daten und testet es auf MariaDB-Daten. Es wurden Hyperparameter eingefügt, die mittels Validation Set konfiguriert wurden.
    ...
    df_static_mariadb, df_static_process_mariadb = lade_daten_mariadb()
    ...
    df_static_mariadb["issue_type_grouped"] = df_static_mariadb["issue_type"].apply(map_issue_type)
    df_static_process_mariadb["issue_type_grouped"] = df_static_process_mariadb["issue_type"].apply(map_issue_type)
    ...
    df_static_mariadb["priority_level"] = df_static_mariadb["priority"].apply(map_priority)
    df_static_process_mariadb["priority_level"] = df_static_process_mariadb["priority"].apply(map_priority)
    ...
    mariadb_static_enc = encode_general_transform(df_static_mariadb, issue_ohe_static, weekday_ohe_static)
    mariadb_static_process_enc = encode_general_transform(df_static_process_mariadb, issue_ohe_static_process, weekday_ohe_static_process)
    ...
    x_static_mariadb = mariadb_static_enc.drop(columns=["cycle_time_days"])
    y_static_mariadb = mariadb_static_enc["cycle_time_days"]
    x_static_process_mariadb = mariadb_static_process_enc.drop(columns=["cycle_time_days"])
    y_static_process_mariadb = mariadb_static_process_enc["cycle_time_days"]
    ...
    y_pred_static = model_static.predict(x_static_mariadb)
    r2_static = r2_score(y_static_mariadb, y_pred_static)
    MAE_static = float(mean_absolute_error(y_static_mariadb, y_pred_static))
    RMSE_static = np.sqrt(mean_squared_error(y_static_mariadb, y_pred_static))
    MedAE_static = median_absolute_error(y_static_mariadb, y_pred_static)
    ...
    y_pred_static_process = model_static_process.predict(x_static_process_mariadb)
    r2_static_process = r2_score(y_static_process_mariadb, y_pred_static_process)
    MAE_static_process = float(mean_absolute_error(y_static_process_mariadb, y_pred_static_process))
    RMSE_static_process = np.sqrt(mean_squared_error(y_static_process_mariadb, y_pred_static_process))
    MedAE_static_process = median_absolute_error(y_static_process_mariadb, y_pred_static_process)
