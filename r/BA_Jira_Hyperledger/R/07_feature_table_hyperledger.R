...
hyperledger_resolved_valid <- hyperledger_resolved_valid %>%
  arrange(key) %>%
...
hyperledger_histories_resolved <- hyperledger_histories_joined %>%
  semi_join(resolved_keys, by = "key") %>%
  filter(!is.na(change_created))

# 3.1) Gesamtaktivität im Changelog---------------------------------------------

changelog_activity <- hyperledger_histories_resolved %>%
  group_by(key) %>%
  summarise(
    changelog_total   = n(),
...
status_changes <- hyperledger_histories_resolved %>%
  filter(field == "status") %>%
  mutate(
...
first_change <- hyperledger_histories_resolved %>%
  group_by(key) %>%
  summarise(
...
inter_event_times <- hyperledger_histories_resolved %>%
...
col_name <- "hyperledger_features_resolved"
