...
mariadb_resolved_valid <- mariadb_resolved_valid %>%
  arrange(key) %>%
...
mariadb_histories_resolved <- mariadb_histories_joined %>%
  semi_join(resolved_keys, by = "key") %>%
  filter(!is.na(change_created))

# 3.1) Gesamtaktivität im Changelog---------------------------------------------

changelog_activity <- mariadb_histories_resolved %>%
  group_by(key) %>%
  summarise(
    changelog_total   = n(),
...
status_changes <- mariadb_histories_resolved %>%
  filter(field == "status") %>%
  mutate(
...
first_change <- mariadb_histories_resolved %>%
  group_by(key) %>%
  summarise(
...
inter_event_times <- mariadb_histories_resolved %>%
...
col_name <- "mariadb_features_resolved"
