...
cat("base_features nach 2):", nrow(base_features), "\n") 
...
status_changes <- jira_histories_resolved %>%
  filter(field == "status") %>%
  mutate(
...
first_change <- jira_histories_resolved %>%
  group_by(key) %>%
  summarise(
...
inter_event_times <- jira_histories_resolved %>%
...
# 5) Missing Values ------------------------------------------------------------
cat("Tickets in features vor Missing-Filter:", nrow(features), "\n")
...
cat("Tickets vor Missing-Filter:", nrow(features), "\n")
cat("Tickets nach Missing-Filter:", nrow(features_clean), "\n")
cat("Insgesamt entfernte Tickets:", nrow(features) - nrow(features_clean), "\n")
cat("Entfernt wegen fehlender Priority:",
    sum(is.na(features$priority) | features$priority == "" | features$priority %in% c("null", "None")),
    "\n")
...
col_name <- "jira_features_resolved"
