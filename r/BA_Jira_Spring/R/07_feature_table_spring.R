...
spring_resolved_valid <- spring_resolved_valid %>%
  arrange(key) %>%
...
spring_histories_resolved <- spring_histories_joined %>%
  semi_join(resolved_keys, by = "key") %>%
  filter(!is.na(change_created))

# 3.1) Gesamtaktivität im Changelog---------------------------------------------

changelog_activity <- spring_histories_resolved %>%
  group_by(key) %>%
  summarise(
    changelog_total   = n(),
...
status_changes <- spring_histories_resolved %>%
  filter(field == "status") %>%
  mutate(
...
first_change <- spring_histories_resolved %>%
  group_by(key) %>%
  summarise(
...
inter_event_times <- spring_histories_resolved %>%
...
col_name <- "spring_features_resolved"
